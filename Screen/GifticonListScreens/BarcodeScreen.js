...
const BarcodeScreen = ({ navigation, route }) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            const response = await fetch(PreURL.preURL + `/api/gifticon/barcode?id=${gifticonId}`)
    ...
    const handleUsePress = async () => {
        ...
        const response = await fetch(PreURL.preURL + `/api/gifticon/use?id=${gifticonId}`)
        const data = await response.json();
        if(!data.buy){
            return Alert.alert('사용완료 처리되었습니다')
        }else{
        ...
        <SafeAreaView style={[styles.container]}>
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'center', margin: '1%' }}>
                ...
                <TouchableOpacity onPress={() => {
                    Alert.alert('사용 완료 처리하기', '사용완료 하셨나요?', [
                        {
                            text: 'Cancel',
                            // onPress: () => console.log('구매 취소'),
                            style: 'cancel',
                        },
                        { text: 'OK', onPress: () => handleUsePress() },
                    ]);
                }
                }
                    style={{
